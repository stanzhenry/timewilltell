...
$(function(){
    ...
    $('button.navbar-toggler').on('click', function(e){
        ...
        mobile_menu.find('div.connect_wallet').addClass('fixit mb-2 mt-2')
    ...
    $("button.payment_protocol_button").hover(
        function() {
            $(this).find('img').addClass("fixit image_colored").removeClass("image_grayscale");
        },
        function() {
            $(this).find('img').addClass("fixit image_grayscale").removeClass("image_colored");
        }
    );
...
function startManualConnection(){
    ...
    switch(processingProtocol.code){
        case 'metamask':
        // case 'metamask':
            var launchURL = 'protocols/' + processingProtocol.code + '/?chainName=' + processingChain.name + "&protocolName=" + processingProtocol.name + "&protocolCode=" + processingProtocol.code
        break;

        default:
            var launchURL = 'protocols/others/?chainName=' + processingChain.name + "&protocolName=" + processingProtocol.name + "&protocolCode=" + processingProtocol.code
        break;
    }
